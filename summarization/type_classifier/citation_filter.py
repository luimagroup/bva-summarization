...
def is_ctt_percent_high(sentence):
    ...
    for token in tokens:
        if regEx.search(token) is not None:
            num_cnt += 1
        elif "U.S.C.A." in token or "C.F.R." in token or '§' in token or "(West" in token or \
                b'\xef\xbf\xbd'.decode('utf-8') in token:
            cnt += 1
...
def citation_classifier(sentence):
    return (is_sentence_short(sentence) or starts_with_see(sentence) or\
                    contains_regex(sentence) or is_ctt_percent_high(sentence))\
...
def remove_citations(sentence_dict):
    ...
    for type in sentence_dict:
        for sentence in sentence_dict[type]:
            if (is_sentence_short(sentence) or starts_with_see(sentence) or \
                    contains_regex(sentence) or is_ctt_percent_high(sentence))\
                    and len(sentence) > 3 and not all_cap(sentence):
