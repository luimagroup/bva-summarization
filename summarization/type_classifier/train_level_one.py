...
# TODO: 0.parse new annotation out in order 1. add citation classifier (legal rule precedes citation) 2. Add sequential information
...
def combine_features(cases_names):
    ...
    for case_name in cases_names:
        ...
        for i in list(range(len(annotated_sentences))):
            ...
            label = 0
            if sent_type == 'EvidenceBasedFinding' or sent_type == 'Evidence':
                label = 1
            elif sent_type == 'Citation':
                continue
            y.append(label)
...
if __name__ == '__main__':
    ...
    pickle.dump(model, open('classifier_level_one.sav', 'wb'))
